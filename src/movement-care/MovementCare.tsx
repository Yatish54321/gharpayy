...
import { appendWorkflowEvent, fetchWorkflowEvents } from "@/lib/workflow-events";
...
function dueForGoal(goal: CareGoal) {
  const minutes =
    goal === "CLOSE" ? 60 : goal === "COMPLETE" ? 90 : goal === "SCHEDULE" ? 120 : 180;
...
export function MovementCare() {
  ...
  const [commitCount, setCommitCount] = useState(
    commitment?.commitCount ?? CARE_PLAYBOOKS[role].stages[0]?.dayCount ?? 1,
  );
  ...
  useEffect(() => {
    void fetchWorkflowEvents("movement-care", undefined, 200)
      .then((items) => setSharedEvents(items.length))
  ...
  const progress = commitment
    ? Math.min(100, Math.round((actual / Math.max(commitment.commitCount, 1)) * 100))
    : 0;
  ...
  const selectedOverdue = Boolean(
    selectedState?.nextAction?.dueAt &&
    new Date(selectedState.nextAction.dueAt).getTime() < Date.now(),
  );
  ...
  useEffect(() => {
    const roleOperator =
      activeRole === "tcm" ? OPERATORS.find((operator) => operator.role === "tcm") : undefined;
  ...
  const acceptDraft = () => {
    ...
    void appendWorkflowEvent({
      module: "movement-care",
      customerId: selectedState.ulid,
      leadId: selectedState.ulid,
      action: "draft_accepted",
      payload: { code, goal: activeGoal, outcome: stage.outcome },
      actor: mv.actor.name,
    }).catch((error: unknown) => console.error("[workflow-events]", error));
  ...
  const startEmptyDraft = () => {
    ...
    setShowManual(true);
    toast.success(
      `Draft clock started — ${manualSize} empty rows, fill them one by one while you work`,
    );
  };
  ...
  const previewMessage = (input: { done: string; wentWell: string; wentBadly: string; problems: string }) => {
    ...
    return debriefMessage({
      ...
      nextStep: selectedState.nextAction
        ? NEXT_ACTION_LABEL[selectedState.nextAction.kind]
        : undefined,
  ...
  const finishDebrief = (input: { done: string; wentWell: string; wentBadly: string; problems: string }) => {
    ...
    mv.log(
      selectedState.ulid,
      "note",
      `${debriefFor.code} wrap-up · done: ${input.done || "—"} · well: ${input.wentWell || "—"} · badly: ${input.wentBadly || "—"} · problem: ${input.problems || "none"}`,
    );
    void appendWorkflowEvent({
      module: "movement-care",
      customerId: selectedState.ulid,
      leadId: selectedState.ulid,
      action: "debrief_captured",
      payload: { debriefId: saved.id, sentOnWhatsapp: false },
      actor: mv.actor.name,
    }).catch((error: unknown) => console.error("[workflow-events]", error));
    return saved;
  };
  const copyMessage = async (id: string, message: string) => {
    try {
      ...
      void appendWorkflowEvent({
        ...
        customerId: selectedState?.ulid,
        leadId: selectedState?.ulid,
        action: "debrief_whatsapp_intent",
        payload: { debriefId: id },
        actor: mv.actor.name,
      }).catch((error: unknown) => console.error("[workflow-events]", error));
  ...
  const dial = () => {
    ...
    void appendWorkflowEvent({
      ...
      leadId: selectedState.ulid,
      action: "call_intent",
      payload: { phone: nameOf.get(selectedState.ulid)?.phone ?? null },
      actor: mv.actor.name,
    }).catch((error: unknown) => console.error("[workflow-events]", error));
  ...
  const aimProperty = (propertyId: string) => {
    ...
    mv.log(
      selectedState.ulid,
      "note",
      `Property to close: ${option.name} · ${option.area} · from ₹${option.fromPrice.toLocaleString("en-IN")}`,
    );
    mv.setNextAction(selectedState.ulid, {
      kind: "send-property",
      dueAt: dueForGoal(activeGoal),
  ...
  const saveReport = () => {
    ...
    void appendWorkflowEvent({
      module: "movement-care",
      action: "round_report",
      payload: { round, actual, committed: commitment.commitCount, moved, stuck, need },
      actor: mv.actor.name,
    }).catch((error: unknown) => console.error("[workflow-events]", error));
    setMoved("");
    setStuck("");
    ...
    <div className="flex h-[calc(100vh-4rem)] min-h-[560px] flex-col overflow-hidden bg-background">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <div className="min-w-0">
            ...
            <p className="text-[10px] text-muted-foreground">
              Draft Vision signal → accountable movement → accepted result
            </p>
          </div>
          <div className="ml-auto flex items-center gap-1 rounded-md border p-0.5">
            {(["flow-ops", "tcm"] as CareRole[]).map((item) => (
              <Button
                key={item}
                size="sm"
                variant={activeRole === item ? "default" : "ghost"}
          ...
          {elapsed ? (
            <Badge
              variant="outline"
              className="h-7 gap-1 border-primary/50 px-2 text-[10px] font-semibold text-primary"
            >
              <Timer className="h-3 w-3" /> Draft running {elapsed} · {manualList.length}/
              {manualSize} filled
              <button type="button" className="ml-1 underline" onClick={stopDraftClock}>
                stop
              </button>
            </Badge>
          ) : (
          ...
          >
            <Hand className="h-3 w-3" /> Draft by hand
            {manualMode ? ` · ${manualList.length}/${manualSize}` : ""}
          </Button>
          <Button
            size="sm"
            variant="outline"
          ...
          <div className="mt-2 grid grid-cols-[minmax(180px,1fr)_repeat(6,minmax(70px,auto))] gap-1.5 overflow-x-auto">
            <div className="min-w-[180px] rounded-md border bg-background px-2 py-1.5">
              <div className="flex items-center justify-between gap-2 text-[10px] font-semibold">
                <span>MY RESULT · {activeGoal}</span>
                <span>
                  {actual}/{commitment.commitCount}
                </span>
              </div>
            ...
            <Stat label="Audit events" value={sharedEvents} />
            ...
            <Stat
              label="Wrap-ups sent"
              value={todaysDebriefs.filter((item) => item.sentOnWhatsapp).length}
            />
            <Stat
              label="At risk"
              value={total.breached + total.p0}
              danger={total.breached + total.p0 > 0}
            />
          </div>
        )}
      </header>
      ...
      {!commitment ? (
        <CommitmentGate
          ...
          onAimProperties={setAimProperties}
          query={propertyQuery}
          onQuery={setPropertyQuery}
          onRole={chooseRole}
          onGoal={chooseGoal}
          onCommitCount={setCommitCount}
          ...
          <section className="flex min-h-0 flex-col overflow-hidden border-r bg-card">
            <div className="border-b px-3 py-2">
              <div className="flex items-center justify-between gap-2">
                ...
                <Badge className={cn("border text-[9px]", GOAL_TONE[activeGoal])}>
                  {activeGoal}
                </Badge>
              </div>
            ...
            <div className="min-h-0 flex-1 divide-y overflow-y-auto">
              {queue.map((item, index) => {
                ...
                return (
                  <Button
                    key={item.ulid}
                    variant="ghost"
                    onClick={() => setSelected(item.ulid)}
                    className={cn(
                      "h-auto w-full justify-start rounded-none px-3 py-2 text-left",
                      selected === item.ulid && "bg-primary/10",
                    )}
                  >
                    ...
                    <span className="min-w-0 flex-1">
                      ...
                      <span
                        className={cn(
                          ...
                          status.accountable ? "text-success" : "text-destructive",
                        )}
                      >
                        {status.result}
                    ...
                    <Badge
                      variant={item.bucket === "P0" ? "destructive" : "outline"}
                      className="text-[9px]"
                    >
                      {item.bucket}
                    </Badge>
              ...
              {manualMode &&
                Array.from({ length: Math.max(0, manualSize - queue.length) }).map((_, index) => (
                  <Button
                    key={`slot-${index}`}
                    variant="ghost"
                    onClick={() => setShowManual(true)}
                  ...
                  >
                    ...
                    <span className="min-w-0 flex-1">
                      ...
                      <span className="block text-[10px] font-normal">
                        Fill it whenever you are ready. Work does not wait.
                      </span>
                    </span>
          ...
          <main className="min-h-0 overflow-y-auto p-2">
            {selectedState && selectedResult ? (
              <div className="space-y-2">
                <div className="border bg-card p-3">
                  <div className="flex flex-wrap items-start gap-2">
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="text-xs text-muted-foreground">
                        {selectedState.lastCustomerMsg ??
                          "Latest WhatsApp message is waiting to be captured."}
                      </p>
                    </div>
                    {selectedOverdue && (
                    ...
                    <Button size="sm" onClick={acceptDraft}>
                      <CheckCircle2 className="h-3.5 w-3.5" /> Draft done · write wrap-up
                    </Button>
                  </div>
                  <div className="mt-3 grid grid-cols-2 gap-1.5 sm:grid-cols-3">
                    ...
                    <ContractCell
                      ...
                      value={selectedState.primaryOwnerName || mv.actor.name}
                      good={Boolean(selectedState.primaryOwnerId)}
                    />
                    <ContractCell
                      label="Deadline"
                      value={
                        selectedState.nextAction
                          ? `${selectedOverdue ? "OVERDUE · " : ""}${new Date(selectedState.nextAction.dueAt).toLocaleString()}`
                          : "Set when draft is accepted"
                      }
                      good={Boolean(selectedState.nextAction) && !selectedOverdue}
                    />
                    <ContractCell label="Proof required" value={stage.proof} />
                    <ContractCell label="Receiver" value={stage.receiver} />
                    <ContractCell
                      label="Acceptance"
                      value={selectedResult.accepted ? "Accepted" : "Not accepted yet"}
                      good={selectedResult.accepted}
                    />
                  </div>
                  {selectedResult.missing.length > 0 && (
                    <div className="mt-2 flex items-center gap-1.5 border-l-2 border-destructive bg-destructive/10 px-2 py-1.5 text-[11px] text-destructive">
                      <AlertTriangle className="h-3.5 w-3.5" /> Not under control: add{" "}
                      {selectedResult.missing.join(", ")}.
                    </div>
                  )}
                  <div className="mt-2 border-t pt-2">
                    ...
                    <p className="mt-1 text-[10px] text-destructive">
                      Does not count: {stage.doesNotCount}
                    </p>
                  </div>
                </div>
                <div className="border bg-card p-3">
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    <p className="text-[10px] font-semibold uppercase text-muted-foreground">
                      Connected call — the result only counts when the customer talks
                    </p>
                    <span className="ml-auto text-[10px] text-muted-foreground">
                      Today {calls.connected} connected of {calls.dialled} dialled · {calls.rate}%
                    </span>
                  </div>
                  <div className="mt-2 flex flex-wrap gap-1.5">
                    <Button size="sm" onClick={dial}>
                      <Phone className="h-3.5 w-3.5" /> Start call{" "}
                      {selectedState.phone ? `· ${selectedState.phone}` : ""}
                    </Button>
                    <Button
                      size="sm"
                      variant="outline"
                      className="border-success/50 text-success"
                      onClick={() => endCall("connected")}
                    >
                  ...
                  <p className="mt-1.5 text-[10px] text-muted-foreground">
                    Work state: {selectedState.work} · last outbound{" "}
                    {selectedState.lastOutboundAt
                      ? new Date(selectedState.lastOutboundAt).toLocaleTimeString()
                      : "none today"}
                  </p>
                </div>
                ...
                <div className="border bg-card p-3">
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    <p className="text-[10px] font-semibold uppercase text-muted-foreground">
                      Property I am aiming to close for this customer
                    </p>
                    <span className="ml-auto text-[10px] font-medium">
                      {selectedState.tourProperty ?? "No property locked yet"}
                    </span>
                  </div>
                  <div className="mt-2 grid gap-1.5 sm:grid-cols-2 lg:grid-cols-3">
                    {rankedForCustomer(selectedState, aimed)
                      .slice(0, 6)
                      .map((option) => (
                        <Button
                          key={option.id}
                          variant="outline"
                          onClick={() => aimProperty(option.id)}
                          className={cn(
                            "h-auto justify-start whitespace-normal p-2 text-left",
                            selectedState.tourProperty === option.name &&
                        ...
                        >
                          <span>
                            ...
                            <span className="block text-[10px] font-normal text-muted-foreground">
                              {option.area} · {option.bedsFree} beds free · from ₹
                              {option.fromPrice.toLocaleString("en-IN")}
                            </span>
                            {aimed.includes(option.id) && (
                              <span className="mt-0.5 block text-[9px] font-semibold text-primary">
                                On today’s closing list
                              </span>
                            )}
                          </span>
                        </Button>
                ...
                {debriefFor?.ulid === selectedState.ulid && (
                  <DebriefCard
                    code={debriefFor.code}
                    customer={nameOf.get(selectedState.ulid)?.name ?? selectedState.ulid}
                    onSave={finishDebrief}
                    onCopy={copyMessage}
              ...
              <div className="flex h-full items-center justify-center text-sm text-muted-foreground">
                Choose a customer to own a result.
              </div>
            )}
          </main>
          ...
          <aside className="min-h-0 overflow-y-auto border-l bg-card p-2">
            ...
            {weakRounds >= 2 && (
              <div className="mt-2 border border-destructive/40 bg-destructive/10 p-2 text-xs">
                ...
                <p className="mt-0.5 text-muted-foreground">
                  Two rounds are weak. Remove or re-route one blocker before continuing.
                </p>
              </div>
            )}
            ...
            <div className="mt-2 border p-2">
              ...
              <p className="mt-1 text-xs font-semibold">
                I will deliver {commitment.commitCount} {stage.unit} today.
              </p>
              <p className="mt-1 text-[10px] text-muted-foreground">{stage.outcome}</p>
              {commitment.supportNeeded && (
                <p className="mt-1 text-[10px]">
                  <strong>Support:</strong> {commitment.supportNeeded}
                </p>
              )}
            </div>
            ...
            <div className="mt-2 border p-2">
              <div className="flex items-center gap-1.5">
                <Building2 className="h-3.5 w-3.5 text-primary" />
                <p className="text-[10px] font-semibold uppercase text-muted-foreground">
                  Properties I am closing today
                </p>
              </div>
              {aimProgress.length === 0 ? (
                <p className="mt-1 text-[10px] text-muted-foreground">
                  No property picked for today. Choose one on any customer to start the closing
                  list.
                </p>
              ) : (
                  ...
                  {aimProgress.map((row) => (
                    <div key={row.id} className="border px-2 py-1.5">
                      ...
                      <p className="text-[9px] text-muted-foreground">
                        {row.area} · {row.bedsFree} beds free
                      </p>
                      <p className="mt-0.5 text-[10px]">
                        Aimed {row.aimed} · tours {row.toursSet} · done {row.toursDone} · booked{" "}
                        <strong className={cn(row.booked > 0 && "text-success")}>
                          {row.booked}
                        </strong>
                      </p>
                    </div>
            ...
            <div className="mt-2 border p-2">
              ...
              {todaysDebriefs.length === 0 ? (
                <p className="mt-1 text-[10px] text-muted-foreground">
                  After each draft, write the wrap-up and paste it in the team group.
                </p>
              ) : (
                <div className="mt-1.5 space-y-1.5">
                    ...
                    <div key={item.id} className="border px-2 py-1.5">
                      <div className="flex items-center justify-between gap-2">
                        <p className="truncate text-[11px] font-semibold">
                          {item.draftCode} · {item.customerName}
                        </p>
                        <Button
                          size="sm"
                          variant="ghost"
                      ...
                      <p className="text-[9px] text-muted-foreground">
                        {item.sentOnWhatsapp ? "Copied for WhatsApp" : "Not sent yet"} ·{" "}
      ...
      <ManualDraftPanel
        ...
        onReplace={replaceInManual}
        onClear={clearManual}
        onCreateLead={createManualLead}
        onFillDemo={fillManualDemo}
        onStartEmpty={startEmptyDraft}
        runningFor={elapsed}
      />
      ...
      {showPlaybook && (
        <PlaybookDrawer playbook={playbook} onClose={() => setShowPlaybook(false)} />
      )}
    </div>
  );
}
function CommitmentGate({
  ...
  const toggleProperty = (id: string) =>
    onAimProperties(
      aimProperties.includes(id)
        ? aimProperties.filter((item) => item !== id)
        : [...aimProperties, id],
    );
  const shown = propertyOptions
    .filter((option) =>
      `${option.name} ${option.area}`.toLowerCase().includes(query.trim().toLowerCase()),
    )
    .slice(0, 12);
      ...
      <div className="mx-auto max-w-5xl border bg-card">
        <div className="border-b px-4 py-3">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-base font-semibold">Set today’s expected result before drafting</h2>
          </div>
          <p className="mt-1 text-xs text-muted-foreground">
            “This is my expectation from today. This is what I will achieve.” Calls and messages are
            work; the selected result is the commitment.
          </p>
        </div>
        <div className="grid gap-4 p-4 lg:grid-cols-[1fr_280px]">
          <div className="space-y-4">
              ...
              <div className="grid gap-2 sm:grid-cols-2">
                {playbook.stages.map((item) => (
                  <Button
                    key={item.goal}
                    variant="outline"
                    onClick={() => onGoal(item.goal)}
                    className={cn(
                      "h-auto min-h-20 justify-start whitespace-normal p-3 text-left",
                      goal === item.goal && GOAL_TONE[item.goal],
                    )}
                  >
                    <span>
                      ...
                      <span className="mt-1 block text-[10px] font-semibold">
                        Usual day: {item.dayCount} {item.unit}
                      </span>
                    </span>
                  </Button>
                ))}
            ...
            <div className="grid gap-3 sm:grid-cols-[200px_1fr]">
              <label className="text-xs font-medium">
                How many {active.unit} today
                <Input
                  type="number"
                  min={1}
                  value={commitCount}
                  onChange={(event) => onCommitCount(Number(event.target.value) || 1)}
                  className="mt-1"
                />
              </label>
              <label className="text-xs font-medium">
                ...
                <Input
                  value={support}
                  onChange={(event) => onSupport(event.target.value)}
                  placeholder="Inventory check, manager help, pricing approval…"
                  className="mt-1"
                />
              </label>
            </div>
            <div>
              <div className="mb-2 flex flex-wrap items-center gap-2">
                <p className="text-[10px] font-semibold uppercase text-muted-foreground">
                  Properties I am aiming to close
                </p>
                <span className="text-[10px] text-muted-foreground">
                  {aimProperties.length} selected
                </span>
                <Input
                  value={query}
              ...
              <div className="grid gap-1.5 sm:grid-cols-2 lg:grid-cols-3">
                {shown.map((option) => (
                  <Button
                    ...
                    className={cn(
                      "h-auto justify-start whitespace-normal p-2 text-left",
                      aimProperties.includes(option.id) && "border-primary bg-primary/10",
                    )}
                  >
                    <span>
                      <span className="block text-[11px] font-semibold">{option.name}</span>
            ...
            <Button onClick={onStart} className="w-full sm:w-auto">
              <Flag className="h-4 w-4" /> Commit this result and open drafts
            </Button>
          </div>
          <div className="border bg-muted/30 p-3">
            ...
            <p className="mt-2 text-sm font-semibold">
              I will deliver {commitCount} {active.unit} today.
            </p>
            ...
            <dl className="mt-3 space-y-2 text-xs">
              ...
              <div>
                <dt className="text-muted-foreground">Does not count</dt>
                <dd className="text-destructive">{active.doesNotCount}</dd>
              </div>
              <div>
                <dt className="text-muted-foreground">Accepted by</dt>
                <dd>{active.receiver}</dd>
              </div>
              <div>
                <dt className="text-muted-foreground">Required when</dt>
                <dd>{active.requireWhen}</dd>
              </div>
              <div>
                <dt className="text-muted-foreground">Closing these properties</dt>
                <dd>
                  {aimProperties.length
                    ? aimProperties
                        .map((id) => propertyOptions.find((option) => option.id === id)?.name)
                        .join(", ")
                    : "Not chosen yet"}
                </dd>
              </div>
      ...
      <div className="mt-2 grid grid-cols-3 gap-1">
        {(Object.keys(ROUND_COPY) as CareRound[]).map((item) => (
          <Button
            key={item}
            size="sm"
            variant={round === item ? "default" : "outline"}
            className="h-7 px-1 text-[9px]"
            onClick={() => onRound(item)}
          >
            {ROUND_COPY[item].label}
          </Button>
        ))}
      </div>
      <p className="mt-2 text-xs font-semibold">{ROUND_COPY[round].question}</p>
      <div className="mt-2 flex items-center justify-between text-[10px]">
        <span>Counted by the system</span>
        <strong>
          {actual}/{committed}
        </strong>
      </div>
      <Progress
        value={Math.min(100, Math.round((actual / Math.max(committed, 1)) * 100))}
        className="mt-1"
      />
      <div className="mt-2 space-y-1.5">
        <Textarea
          value={moved}
          onChange={(event) => onMoved(event.target.value)}
          placeholder="Moved — what result changed?"
          className="min-h-14 text-xs"
        />
        <Textarea
          value={stuck}
          onChange={(event) => onStuck(event.target.value)}
          placeholder="Stuck — what is blocking the result?"
          className="min-h-14 text-xs"
        />
        <Input
          value={need}
          onChange={(event) => onNeed(event.target.value)}
          placeholder="Need — who should help with what?"
          className="h-8 text-xs"
        />
      </div>
      <Button size="sm" className="mt-2 w-full" onClick={onSave}>
        Report progress
      </Button>
...
function PlaybookDrawer({
  playbook,
  onClose,
}: { playbook: (typeof CARE_PLAYBOOKS)[CareRole]; onClose: () => void }) {
  return (
    <div className="absolute inset-y-0 right-0 z-40 w-full max-w-md overflow-y-auto border-l bg-card shadow-xl">
      <div className="sticky top-0 flex items-center justify-between border-b bg-card px-4 py-3">
        <div>
          <p className="text-[10px] uppercase text-muted-foreground">CARE V5 playbook</p>
          <h2 className="font-semibold">{playbook.label}</h2>
        </div>
      ...
      <div className="space-y-3 p-4">
        <div className="border p-3">
          <p className="text-[10px] font-semibold uppercase text-muted-foreground">Role promise</p>
          <p className="mt-1 text-sm">{playbook.promise}</p>
        </div>
        {playbook.stages.map((item) => (
          <div key={item.goal} className="border p-3">
            <div className="flex items-center justify-between gap-2">
              ...
              <span className="text-xs font-semibold">
                A usual day: {item.dayCount} {item.unit}
              </span>
            </div>
            ...
            <div className="mt-2 grid gap-1.5 text-xs">
              <p>
                <strong>It counts when:</strong> {item.proof}
              </p>
              <p className="text-destructive">
                <strong>It does not count when:</strong> {item.doesNotCount}
              </p>
              <p>
                <strong>Handed to:</strong> {item.receiver}
              </p>
              <p>
                <strong>Choose this day when:</strong> {item.recommendWhen}
              </p>
              <p>
                <strong>You must choose it when:</strong> {item.requireWhen}
              </p>
            </div>
          </div>
        ))}
        <div className="border p-3">
          ...
          <p className="mt-1 text-xs">{playbook.acceptanceGate}</p>
        </div>
        <div className="border p-3">
          <p className="text-[10px] font-semibold uppercase text-muted-foreground">
            Non-negotiable safeguards
          </p>
          {playbook.safeguards.map((item) => (
            <p key={item} className="mt-2 flex gap-2 text-xs">
...
function DebriefCard({
  ...
  onSave: (input: { done: string; wentWell: string; wentBadly: string; problems: string }) =>
    { id: string; message: string } | undefined;
  onCopy: (id: string, message: string) => void; onClose: () => void;
}) {
    ...
    <div className="border-2 border-primary bg-card p-3">
      <div className="flex flex-wrap items-center gap-2">
        ...
        <p className="text-xs font-semibold">
          {code} is done for {customer} — wrap it up before you move on
        </p>
        <Button
          size="sm"
          variant="ghost"
      ...
      <div className="mt-2 grid gap-1.5 sm:grid-cols-2">
        <label className="text-[10px] font-semibold uppercase text-muted-foreground">
          ...
          <Textarea
            ...
            placeholder="Called, qualified, shared 2 properties…"
        ...
        <label className="text-[10px] font-semibold uppercase text-muted-foreground">
          ...
          <Textarea
            ...
            placeholder="Customer picked a date straight away…"
            className="mt-1 min-h-14 text-xs"
          />
        </label>
        <label className="text-[10px] font-semibold uppercase text-muted-foreground">
          ...
          <Textarea
            value={wentBadly}
            onChange={(event) => setWentBadly(event.target.value)}
            placeholder="Budget below our price, went cold on rent…"
            className="mt-1 min-h-14 text-xs"
          />
        </label>
        <label className="text-[10px] font-semibold uppercase text-muted-foreground">
          Any other problem or help needed?
          <Textarea
            value={problems}
            onChange={(event) => setProblems(event.target.value)}
            placeholder="Need inventory truth for Salarpuria, need pricing approval…"
            className="mt-1 min-h-14 text-xs"
          />
        </label>
      </div>
      <div className="mt-2 border bg-muted/30 p-2">
        <p className="text-[10px] font-semibold uppercase text-muted-foreground">
          WhatsApp message being written — live
        </p>
        <pre className="mt-1 whitespace-pre-wrap break-words text-[11px] leading-snug">
          {onPreview({ done, wentWell, wentBadly, problems })}
        </pre>
      </div>
      ...
      {saved && (
        <div className="mt-2 border bg-muted/30 p-2">
          <p className="text-[10px] font-semibold uppercase text-muted-foreground">
            Copy this and paste it in the team WhatsApp group
          </p>
          <pre className="mt-1 whitespace-pre-wrap break-words text-[11px] leading-snug">
            {saved.message}
          </pre>
      ...
      <div className="flex h-full w-full max-w-lg flex-col border-l bg-card shadow-xl">
        <header className="flex shrink-0 items-center gap-2 border-b px-3 py-2">
          ...
          <div>
            ...
            <p className="text-[10px] text-muted-foreground">
              This exact message is built after every draft is done.
            </p>
          </div>
          <Button
            size="sm"
...
📊 My day so far: <done>/<promised> results`}</pre>
          ...
          <div className="border p-2">
            <p className="text-[10px] font-semibold uppercase text-muted-foreground">
              Live example with your numbers
            </p>
            <pre className="mt-1 whitespace-pre-wrap break-words text-[11px] leading-snug">
              {sample}
            </pre>
          ...
          <p className="text-[10px] text-muted-foreground">
            Property and next step appear only when they exist on the customer. Nothing else is
            added automatically.
          </p>
        </div>
      </div>
...
function Stat({ label, value, danger }: { label: string; value: string | number; danger?: boolean }) {
  return (
    <div className="min-w-[70px] rounded-md border bg-background px-2 py-1">
      <p className="text-[9px] text-muted-foreground">{label}</p>
      <p className={cn("text-sm font-semibold", danger && "text-destructive")}>{value}</p>
    </div>
  );
}
function ContractCell({ label, value, good }: { label: string; value: string | number; good?: boolean }) {
  return (
    <div className="min-h-14 border px-2 py-1.5">
