...
interface Props { lead: MovementState; onLogged?: () => void; }
export function CallEngine({ lead, onLogged }: Props) {
  ...
  const deadlineLate = Boolean(
    lead.nextAction?.dueAt && new Date(lead.nextAction.dueAt).getTime() < Date.now(),
  );
  ...
  function commit() {
    ...
    mv.log(
      lead.ulid,
      "note",
      `${def.label} · ${MOVEMENT_LABEL[outputs.movement]}${cap.note ? ` — ${cap.note}` : ""}`,
    );
    const record = {
    ...
    toast.success(
      `${def.label} logged · ${MOVEMENT_LABEL[outputs.movement]} · next: ${outputs.nextStep.label}`,
    );
    setPhase("mission");
    setCap(emptyCapture());
    setOutputs(null);
    ...
    <div className="space-y-3 rounded-lg border p-3">
      ...
      {phase === "capture" && (
        <>
              ...
              {(agenda === "qualification" || agenda === "follow-up") && (
                <>
                  <div>
                    ...
                    <div className="mt-1 flex flex-wrap gap-1">
                      {([
                        ...
                        <Chip
                          key={label}
                          on={cap.moveIn === isoInDays(d)}
                          onClick={() => set({ moveIn: isoInDays(d) })}
                        >
                          {label}
                        </Chip>
                      ))}
                  ...
                  <div>
                    <Title>Budget</Title>
                    <div className="mt-1 flex flex-wrap gap-1">
                      {BUDGET_OPTIONS.map(([label, v]) => (
                        <Chip key={label} on={cap.budget === v} onClick={() => set({ budget: v })}>
                          {label}
                        </Chip>
                      ))}
                    </div>
          ...
          />
          <Button className="w-full" onClick={() => finish(outcome)}>
            End call · build message, follow-up and next step
          </Button>
        </>
      )}
        ...
        <>
          ...
          <div className="rounded-md border p-2">
            ...
            <div className="mt-1 text-[10px] text-muted-foreground">
              Captured work: {cap.activities.length} activities · {cap.promises.length} promises ·{" "}
              {cap.note || "no note"}
            </div>
          </div>
...
function VerifyPanel({
  ...
  lead: MovementState;
  cap: CallCapture;
  set: (patch: Partial<CallCapture>) => void;
}) {
  const q = lead.q ?? {};
  ...
  const rows = [
    {
      ...
      onChange: (v: string) => set({ moveIn: v ? new Date(v).toISOString() : null }),
    },
    {
      label: "Area",
      crm: q.location ?? "",
      live: cap.area ?? "",
      onChange: (v: string) => set({ area: v }),
    },
    {
      label: "Office / College",
      crm: q.officeOrCollege ?? "",
      live: cap.officeOrCollege ?? "",
      onChange: (v: string) => set({ officeOrCollege: v }),
    },
    {
      label: "Budget",
      crm: q.budget ? String(q.budget) : "",
      live: cap.budget ? String(cap.budget) : "",
      onChange: (v: string) => set({ budget: Number(v) || null }),
    },
    {
      label: "Room",
      crm: q.roomType ?? "",
      ...
      live:
        cap.inBangalore === null || cap.inBangalore === undefined
          ...
          : cap.inBangalore
            ? "Yes"
            : "No",
      onChange: (v: string) => set({ inBangalore: /^y/i.test(v) ? true : /^n/i.test(v) ? false : null }),
    },
    {
      label: "For whom",
      crm: q.forSelf === null || q.forSelf === undefined ? "" : q.forSelf ? "Self" : "Someone else",
      live: cap.forWhom ?? "",
      onChange: (v: string) => set({ forWhom: (v || null) as CallCapture["forWhom"] }),
    },
  ];
  return (
...
const fmtDate = (iso: string | number | Date | null | undefined) =>
